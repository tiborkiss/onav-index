...
- **Tier 1 (live):** `### Referenced by` is a Dataview query — backlinks sorted by type, re-evaluated on every open, each shown as a self-aliased `ID — Title` link. New entities linking to this one appear with no regen. (The native backlinks panel is the no-plugin fallback.)
