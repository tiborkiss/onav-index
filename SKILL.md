...
- **Path-qualified wikilinks prevent cross-project linking.** When multiple BMad projects share the same entity ID scheme (FR-1.md exists in each), bare `[[FR-1]]` resolves ambiguously — Obsidian may link to the wrong project. Every reference in the static `### References` section is path-qualified: `[[projects/.../FR/FR-1|FR-1]]`. The alias keeps the display text clean (`FR-1`), the path makes the target unambiguous. Dataview queries are separately folder-scoped via `FROM [[]] AND "projects/..."`.
